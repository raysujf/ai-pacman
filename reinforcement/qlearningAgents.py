...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in actions:
            if Q==self.getQValue(state,action):result.append(action)
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        before=self.values[(state,action)]
        after=reward+(self.discount*self.computeValueFromQValues(nextState))
        self.values[(state,action)]=(1-self.alpha)*before+self.alpha*after
