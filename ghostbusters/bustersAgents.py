...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        pacman=[]
        for i in legal:
            pacman.append(Actions.getSuccessor(pacmanPosition,i))

        distance=float("inf")
        for i in livingGhostPositionDistributions:
            ghost=find_max(i)
            temp=self.distancer.getDistance(ghost, pacmanPosition)
            if distance>temp:
                distance=temp
                closed=ghost
        ...
        for i in range(len(pacman)):
            temp=self.distancer.getDistance(closed,pacman[i])
            if temp<result:
                result=temp
                end=legal[i]
